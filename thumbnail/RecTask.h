...
class  CRecTask
{
	...
	BOOL VaildCellType(PULONG pActiveCell, ULONG nActiveCounter, ULONG nTestCellType);
...
public:
	...
	KSTATUS   GetPickupRecCellGrap(PCHAR  pImageFolder, USHORT  nGrapIndex, UCHAR  nGrapSize, PCELL_FULL_INFO_EX &cell_info_ex,ULONG  m_CellTypeCounter,PULONG  m_CellTypeArray,BOOL  bBuildMap = TRUE); //注意:cell_info使用完后,需要调用FreeCellFullInfo进行free
