...
{"CaDrvQueryConfig"}, {"CaDrvSetConfig"},{"CaDrvReadRawBuffer"},{"CaDrvCatchGrap"}, {"CaDrvSetOutputCB"},{"CaDrvCloseAll"},{"CaDrvSetWBalance"}/*,{"CaDrvCatchBitmap"}*/};
...
KSTATUS CCameraMgr::SetPrviewWindow( UCHAR nGlobalCaIndex, HWND hPrvWnd, RECT nPrvRC )
{
	...
	DBG_MSG("CCameraMgr::SetPrviewWindow nRelativeCaIndex:%d, hPrvWnd:%x, nPrvRC:(%d,%d,%d,%d) \n", nRelativeCaIndex, hPrvWnd, nPrvRC.left, nPrvRC.top, nPrvRC.right, nPrvRC.bottom);
...
KSTATUS CCameraMgr::ReadRawGrapBuffer( USHORT nGlobalCaIndex, HANDLE *pHandle )
{
	...
	{
		DBG_MSG("CCameraMgr::ReadRawGrapBuffer->Exception...[%s]\n", e.what());
		return STATUS_EXCEPTION_ERROR;
	}
}
...
KSTATUS CCameraMgr::CatchGrap( USHORT nCaIndex,PCHAR pGrapPath,HANDLE hRawGrapBuffer )
{
	...
	{
		DBG_MSG("CCameraMgr::CatchGrap->Exception...[%s]\n", e.what());
		return STATUS_EXCEPTION_ERROR;
	}
}
...
KSTATUS CCameraMgr::CloseAllDevice()
{
	...
	for( ULONG j = 0 ; j < m_CaInfo.nDevCounter ; j ++ )
		//for( ULONG j =  m_CaInfo.nDevCounter - 1 ; j >= 0 ; j -- )
	{
		if( DEV_START(&m_CaInfo.pCaDevice[j].bStart) )
		{
			try
...
KSTATUS CCameraMgr::CatchBitmap(USHORT nCaIndex,HANDLE *pHbitmap,HANDLE hRawGrapBuffer)
{
	USHORT        nRelativeCaIndex;
	KSTATUS       status;
	PCA_DEVICE    ca_dev;
	...
	try
	{
		return ca_dev->pRelativeDriver->nDrvAPI.DrvCatchBitmap(nRelativeCaIndex, pHbitmap, hRawGrapBuffer);
	}
	catch (std::exception &e)
	{
		DBG_MSG("CCameraMgr::CatchBitmap->Exception...[%s]\n", e.what());
		return STATUS_EXCEPTION_ERROR;
	}
}
